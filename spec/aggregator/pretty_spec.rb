...
RSpec.describe Drunker::Aggregator::Pretty do
  ...
  describe "#run" do
    before do
      allow(builder1).to receive(:build_id).and_return("build_id_1")
      allow(builder2).to receive(:build_id).and_return("build_id_2")
      allow(builder1).to receive(:success?).and_return(true)
      allow(builder2).to receive(:success?).and_return(false)
      allow(artifact).to receive(:output).and_return("build_id_1" => "Success!", "build_id_2" => "Failed...")
    end
